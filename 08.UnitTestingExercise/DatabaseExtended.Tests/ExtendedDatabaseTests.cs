...
namespace DatabaseExtended.Tests
{
    ...
    public class ExtendedDatabaseTests
    {
        ...
        private const string AddMaxNumExpectedException = "Array's capacity must be exactly 16 integers!";
        ...
        private const string FindByUsernameNullException = "Username parameter is null!";
        ...
        public void Add_DatabaseIsFull_ThrowsException()
        {
            ...
            Exception ex = Assert.Throws<InvalidOperationException>(() => sut.Add(new Person(333333, "Petkan")));
            Assert.That(ex.Message, Is.EqualTo(AddMaxNumExpectedException));
        }

        [Test]
        public void Add_UserNameExists_ThrowsException()
        {
            Exception ex = Assert.Throws<InvalidOperationException>(() => sut.Add(new Person(333333, ValidUserNamePesho)));
            Assert.That(ex.Message, Is.EqualTo(AddUserNameExistsExpectedException));
        }

        [Test]
        public void Add_UserIdExists_ThrowsException()
        {
            Exception ex = Assert.Throws<InvalidOperationException>(() => sut.Add(new Person(ValidIdIvan, "Tedo")));
        ...
        [Test]
        public void FindByUserName_UserNameNull_ReturnsUser()
        {
            ArgumentNullException ae = Assert.Throws<ArgumentNullException>(() => sut.FindByUsername(null));
            Assert.That(ae.ParamName, Is.EqualTo(FindByUsernameNullException));
        }

        [Test]
        public void FindByUserName_UserIsWhiteSpace_ReturnsUser()
        {
            InvalidOperationException ae = Assert.Throws<InvalidOperationException>(() => sut.FindByUsername(" "));
        }
        ...
        {
            ArgumentOutOfRangeException ae = Assert.Throws<ArgumentOutOfRangeException>(() => sut.FindById(-1));
        }

        [Test]
        public void FindById_UserDoesNotExist_ThrowsException()
        {
            InvalidOperationException ae = Assert.Throws<InvalidOperationException>(() => sut.FindById(999));
        }
